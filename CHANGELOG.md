...
- **Restored partial recovery of a died/failed remote job's on-disk output** (`RemoteJobExecutor._salvage_partial_output`, `remote_submission/executor.py`), called from `_run_single_job`'s failure handler. ExPyRe's own `output_files` stage-out only runs for jobs that finish cleanly, so a hard crash (missing GPU driver, OOM-kill, a numerically unstable MD structure) previously discarded that job's output entirely even when the data genuinely existed — `get_remotes()`'s periodic mid-run sync pulls a job's *entire* remote stage directory back regardless of eventual status. Reuses ExPyRe's own `_expyre_output_files` marker and `ExPyRe._copy` to recover whatever exists. Matters most for `structure_generation`: a crash mid-MD-run would otherwise silently drop the whole trajectory, discarding exactly the kind of high-uncertainty structure active learning most needs.
- **`run_md` (`structure_generation/md/md_wfl.py`) now catches exceptions during force evaluation and the dynamics step**, not just the pre-existing `forces > 1000 eV/Å` check — a numerically unstable structure (e.g. NPT's `LangevinBAOAB` hitting a singular cell matrix) now stops that MD run gracefully, keeping whatever was already written to the trajectory, instead of killing the whole remote job with nothing recovered. Also fixed the existing force check to catch non-finite (NaN/Inf) forces, which a bare `> 1000` comparison silently let through (`np.nan > 1000` is `False`).
- **Near-total MACE prediction failures are no longer silently invisible.** `_save_mace_eval_predictions` (`mlip/mace_wfl.py`) previously logged a per-structure prediction failure at `logger.debug` — but that call runs in a freshly-spawned remote process where `setup_logging()` is never called, so there was no handler for DEBUG-level records, and `RemoteJobExecutor._run_single_job` discarded a successful job's `stdout`/`stderr` entirely. Together these made a near-total prediction failure (1 succeeding out of 1405 structures, observed in production) completely silent, degrading parity plots to a single trivial point with no error anywhere to explain why. The first failure per tag now logs at `WARNING` with a full traceback (subsequent identical failures drop to `DEBUG` to avoid flooding the log), a summary line reports the total success/failure count, and `_run_single_job` now logs a successful job's `stdout`/`stderr` at `DEBUG` instead of discarding them.
- Silenced expyre's per-retry-attempt `FailedSubprocessWarning` specifically for the known, harmless race between mid-training rsync (polled every `check_interval` while an `mlip_committee` job is still running) and MACE's own checkpoint housekeeping (which deletes the previous checkpoint right after writing the new one) — expyre's own subprocess retry already recovers from the resulting transient "stale file handle"/"file has vanished" errors; only the retry-in-progress warnings are filtered, scoped narrowly to `rsync` so unrelated subprocess retries (e.g. flaky ssh) stay visible.

### Changed
- **`RemoteInfo.lock_timeout` (the cap on how long a `RemoteJobExecutor` worker thread waits to acquire the per-host ssh-call lock) defaults to `None` again** (wait indefinitely), reverting a 0.5.2 change that derived it from the job's `max_time`. The thread genuinely holding the lock and blocked on an interactive ssh password/OTP prompt was never bounded either way — deriving a wait-for-lock timeout only turned "everything queued behind it waits quietly" into "everything else fails loudly around the one thing that's still just waiting." `ensure_ssh_connectivity` (above) is the actual fix for the underlying problem this was trying to guard against.
...
### Added
- **`alomancy upgrade-hpc` CLI command** (`cli/upgrade_hpc.py`): interactive wizard that upgrades the installed `alomancy` package on one or more configured HPC systems by running `pip install --upgrade alomancy` over ssh. Lists profiles from `~/.alomancy/hpc_config.yaml`, accepts a comma-separated selection or `all`, and reports per-host success/failure without aborting the batch over one bad profile. Shares its ssh/host-resolution helpers (`resolve_hpc_host`, `run_ssh_command`, `derive_python_from_venv`) with the workflow-startup remote-version check via the new `utils/remote_ssh.py` module.
