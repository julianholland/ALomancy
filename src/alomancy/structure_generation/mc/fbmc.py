...
def run_fbmc(
    ...
    fbmc_object = ForceBias(
        ...
        delta=delta,
        trajectory=str(Path(out_dir, f"{structure_generation_job_dict['name']}.traj")),
        logfile=str(Path(out_dir, f"{structure_generation_job_dict['name']}.log")),
        append_trajectory=False,
        loginterval=snapshot_interval,
    )

    fbmc_object.run(steps)

    atom_traj_list = Trajectory(
        str(Path(out_dir, f"{structure_generation_job_dict['name']}.traj")), "r"
    )
    if verbose > 0:
        print(
...
if __name__ == "__main__":
    model_path = Path("test_stagetwo.model")
    run_fbmc(
        structure_generation_job_dict={
            "name": "fbmc_test",
