...
def _save_mace_eval_predictions(
    ...
    model_path = (
        regular_model_path if regular_model_path.exists() else compiled_model_path
    )
    if not model_path.exists():
        model_path = Path(f"{name}.model")
    if not model_path.exists():
        logger.warning("Model not found; skipping eval predictions.")
        return
