...
def create_dimer_atoms_list(element_a: str, element_b: str, num_dimers: int) -> list:
    ...
    dimer_atoms = []
    vdw_list = [
        vdw_radii[atomic_numbers[element_a]],
        vdw_radii[atomic_numbers[element_b]],
    ]
...
def create_trimer_atoms_list(
    element_a: str, element_b: str, element_c: str, num_trimers: int
) -> list:
    ...
    vdw_list = [
        vdw_radii[atomic_numbers[element_a]],
        vdw_radii[atomic_numbers[element_b]],
        vdw_radii[atomic_numbers[element_c]],
    ]
    distance_range = (0.2 * sum(vdw_list), 1.0 * sum(vdw_list))
    cell = [distance_range[1] * 3] * 3  # Ensure enough space
    if num_trimers > 1:
        for dx in np.linspace(
            distance_range[0],
            distance_range[1],
            int(np.floor(np.sqrt(num_trimers - 2))),
        ):
        ...
        for dx in np.linspace(
            distance_range[0], distance_range[1], num_trimers - len(trimer_atoms)
        ):
            a1 = Atoms(element_a, positions=[(0, 0, 0)])
