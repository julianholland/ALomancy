...
**Per-host ssh-call serialization**: `job.start()` (mkdir/stage-input/sbatch submit) and `job.get_results()`'s polling-loop sync (`sync_remote_results_status()` — squeue-equivalent status + rsync) both shell out over a given HPC host's shared multiplexed ssh control connection. Letting `max_concurrent_jobs` worker threads hit either one simultaneously on the same host can exceed whatever session cap the remote sshd enforces, at which point the excess sessions silently fall back to a fresh, separately-authenticated connection that hangs forever if that auth needs interactive input nobody is present to provide. `executor._get_ssh_call_lock(sys_name)` hands out one lock per HPC `system_name` (not one global lock), created lazily and cached in a module-level dict guarded by a short-lived dict-lookup lock; two `RemoteJobExecutor`s targeting different hosts never serialize against each other's ssh traffic. `_ensure_expyre_sync_serialized()` (called once per `run_all_jobs_bounded`) monkeypatches `ExPyRe.sync_remote_results_status` to acquire the calling job's host lock before syncing, and coalesces redundant syncs via a per-host generation counter (`_sync_generations`) — since every call here uses ExPyRe's `sync_all=True` default, one thread's sync already covers every job on that host, so a thread that waited for the lock skips its own call if another thread already synced that host in the meantime. Each lock is only held for the moment of the actual subprocess call, not `get_results()`'s `check_interval` sleeps, so each job's own polling cadence — and real completion order — stays independent.

**`lock_timeout`**: `RemoteInfo.lock_timeout` (set by `get_remote_info` from the job's own `max_time`/`max_go_time` via `expyre.units.time_to_sec`) bounds how long a thread will wait to acquire its host's ssh-call lock before raising `TimeoutError` instead of blocking forever. A wait longer than the job's entire expected walltime just to get a turn to touch ssh means the lock holder is genuinely stuck — most likely the shared control connection died and a fresh one needs interactive auth nobody can provide — so the waiting job fails loudly and promptly rather than hanging alongside it. `None` (the `RemoteInfo` constructor default) waits indefinitely, matching pre-timeout behavior. `_run_single_job` stashes the timeout on the job instance (`job._alomancy_lock_timeout`) right after `start()` so the later `sync_remote_results_status` monkeypatch — which only receives `self` — can reuse the same bound.
