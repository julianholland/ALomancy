...
- **`RemoteJobExecutor`'s ssh-call serialization is thread-scoped, not process-scoped**: `_get_ssh_call_lock`/`_ensure_expyre_db_thread_safe` (`remote_submission/executor.py`) use `threading.Lock`, which only serializes worker threads within one Python process. On its own this gives no protection if multiple separate alomancy processes mutate the same `jobs.db` at once — `expyre`'s own `list(config.db.jobs(id=...))[0]` pattern (`func.py`) and squeue-line parsing (`schedulers/slurm.py`) can raise a bare `IndexError` in that scenario. Two things now guard against it (see the next two bullets): per-run isolation removes the shared file in the first place, and `acquire_local_expyre_lock` catches same-directory double-launches that isolation alone doesn't. `_run_single_job`'s failure handler additionally logs the full traceback at DEBUG level (`results/alomancy.log`) so any *other* trigger of that same `IndexError` (e.g. a stray blank line in squeue's captured stdout — confirmed to still occur even from a single, non-concurrent process) can be root-caused from the log instead of needing source-level archaeology.
