...
- `train_mlip`'s committee backfill (fires when fewer than 3 trained models are found — 3 is the floor for a usable committee std-dev) retrains the specific missing `fit_idx` values via `committee_remote_submitter(..., fit_indices=[...])`, never `range(size_of_committee)` from 0 — the latter would silently overwrite already-successful low-index fits instead of the ones that actually failed, since `mace_fit` derives its output directory purely from `fit_idx`. The phase is only marked done if at least 3 models exist after retrying; otherwise `train_mlip` raises rather than proceeding with an unusable committee.
